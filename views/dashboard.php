...
<div class="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl md:text-3xl font-display font-bold text-navy">Halo, <?= htmlspecialchars($user['nama']) ?> 👋</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <?php 
        $cards = [
            ['Total Obat', $stats['total'], 'bi-capsule', 'text-navy', 'bg-slate-50'],
            ['Stok Kritis', $stats['kritis'], 'bi-exclamation-triangle', 'text-amber-500', 'bg-amber-50'],
            ['Habis', $stats['habis'], 'bi-x-octagon', 'text-rose-500', 'bg-rose-50'],
            ['Kadaluarsa', $stats['expired'], 'bi-calendar-x', 'text-rose-600', 'bg-rose-100']
        ];
        foreach ($cards as [$label, $val, $icon, $color, $bg]): ?>
            <div class="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex items-center justify-between hover:shadow-md transition-all group">
                <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <tbody id="stock-tbody" class="divide-y divide-slate-50">
                            ...
                            ?>
                                <tr class="hover:bg-slate-50 transition group">
                                    <td class="px-8 py-5 font-bold text-navy group-hover:text-emerald transition-colors"><?= htmlspecialchars($o['nama']) ?></td>
                                    <td class="px-8 py-5">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8">
                <h2 class="font-bold text-xl text-navy mb-6">Aktivitas Terakhir</h2>
                <div class="space-y-6">
